...
import { Check, X, Terminal, Loader } from 'lucide-react'
...
export default function Remediation() {
  ...
  async function approve(id: number) {
    ...
    setSaveError(s => { const n = { ...s }; delete n[id]; return n })
    try {
      await api.remediation.updateStatus(id, 'approved')
    } catch (e: any) {
      setLocalStatus(s => ({ ...s, [id]: 'pending' }))
      setSaveError(s => ({ ...s, [id]: e.message }))
    } finally {
  ...
  async function deny(id: number) {
    ...
    setSaveError(s => { const n = { ...s }; delete n[id]; return n })
    try {
      await api.remediation.updateStatus(id, 'denied')
    } catch (e: any) {
      setLocalStatus(s => ({ ...s, [id]: 'pending' }))
      setSaveError(s => ({ ...s, [id]: e.message }))
    } finally {
      ...
      {loading ? <Spinner /> : filtered.length === 0 ? (
        ...
        <div className="step-list">
          {filtered.map(step => {
            ...
            const isSaving = saving[step.id] ?? false
            const stepError = saveError[step.id]
                ...
                <div className="step-body">
                  ...
                  {stepError && (
                    <div style={{
                      ...
                      padding: '6px 10px', borderRadius: 'var(--radius)', marginBottom: 8,
                    }}>
                      Failed to save: {stepError}
                    </div>
                  )}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    {isPending && (
                      <div className="step-actions" style={{ marginTop: 0, paddingTop: 0, borderTop: 'none' }}>
                        <button className="btn btn-approve" disabled={isSaving} onClick={() => approve(step.id)}>
                          {isSaving ? <Loader size={13} style={{ animation: 'spin 0.8s linear infinite' }} /> : <Check size={13} />}
                          Approve
                        </button>
                        <button className="btn btn-deny" disabled={isSaving} onClick={() => deny(step.id)}>
                          {isSaving ? <Loader size={13} style={{ animation: 'spin 0.8s linear infinite' }} /> : <X size={13} />}
                          Deny
                        </button>
                      </div>
                    )}
                    ...
                    {isSaving && (
                      <span className="flex gap-8 items-center" style={{ fontSize: 12, color: 'var(--ink-3)', fontFamily: 'var(--font-mono)' }}>
